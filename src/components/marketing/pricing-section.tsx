...
export function PricingSection({ showHeading = true }: { showHeading?: boolean }) {
  return <Section id="pricing"><Container>
    ...
    <div className="pricing-trial"><details><summary>What’s included in the free trial?</summary><p>{trialLimits.keyword_expand} keyword searches, {trialLimits.annotation_explorer} idea searches, {trialLimits.top_pins} Top Pins search, and {trialLimits.research_project} research project per day. Up to {trialLimits.max_pins_per_search} pins per search. Trial usage limits apply.</p></details><p>Available payment options are shown in the app. If checkout is unavailable, contact support to discuss plan activation. For cancellation or billing help, email <a className="text-link" href="mailto:support@pinitgrow.com">support@pinitgrow.com</a>.</p><p><Link className="text-link" href="/terms">Terms of Service</Link> · <Link className="text-link" href="/refund">Refund & cancellation policy</Link> · <Link className="text-link" href="/privacy">Privacy Policy</Link></p>{showHeading && <Link href="/pricing#compare" className="text-link">Compare every plan detail <ArrowRight /></Link>}</div>
  </Container></Section>;
}
