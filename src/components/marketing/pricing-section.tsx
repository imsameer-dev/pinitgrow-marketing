...
const summaryRows = ["Keyword Explorer", "Top Pins credits", "Maximum pins/search", "Stored projects", "Active trackers", "Tracker history"];
  ...
  return <Section id="pricing"><Container>
    ...
    <div className="grid items-stretch gap-4 lg:grid-cols-3">
      {plans.map((plan) => <article key={plan.code} className={cn("pricing-plan relative flex flex-col border border-border bg-card", plan.featured && "pricing-plan-featured")}>
        ...
        <p className="mb-6 mt-2 text-xs text-muted-foreground">USD · billed monthly</p>
        ...
        <p className="mt-7 border-b border-border pb-3 text-xs font-semibold">Your monthly research capacity</p>
    ...
    <div className="pricing-trial"><p>Monthly credits reset on your subscription anniversary and do not roll over. One Top Pins credit covers up to 100 pins; larger fresh searches use 2–5 credits. Active trackers share one allowance across Rank and Search Tracker, with daily automatic checks included. CSV exports and cached research are unlimited. All plans are single-login workspaces; shared team seats are not included.</p><p>{trialDays} days to explore. No credit card required. Cancel your monthly plan anytime.</p><details><summary>What’s included in the free trial?</summary><p>{trialLimits.keyword_expand} keyword searches, {trialLimits.annotation_explorer} idea searches, {trialLimits.top_pins} Top Pins search, and {trialLimits.research_project} research project per day. Up to {trialLimits.max_pins_per_search} pins per search. Trial usage limits apply.</p></details>{showHeading && <Link href="/pricing#compare" className="text-link">Compare every plan detail <ArrowRight /></Link>}</div>
  </Container></Section>;
}
