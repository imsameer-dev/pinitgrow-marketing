...
const summaryRows = ["Keyword Explorer", "Top Pins searches", "Maximum pins/search", "Stored projects", "Active trackers", "Tracker history"];
  ...
  return <Section id="pricing"><Container>
    ...
    <div className="grid items-stretch gap-4 lg:grid-cols-3">
      {plans.map((plan) => <article key={plan.code} className={cn("pricing-plan relative flex flex-col border border-border bg-card", plan.featured && "pricing-plan-featured")}>
        <div className="flex items-center justify-between gap-2"><h3 className="text-lg font-semibold">{plan.name}</h3>{plan.featured && <span className="rounded-sm bg-foreground px-2 py-1 text-[10px] font-medium text-white">For growing teams</span>}</div>
        ...
        <p className="mt-7 border-b border-border pb-3 text-xs font-semibold">Your research capacity</p>
